...
settings["analyzed_data_dir"] = "../../data/p5_encodingmodel/full_model/"
...
for r_nr,rec in enumerate(recs):
    ...
    print("    Running {}x cross-validated regression on category model (L1={})...".format(settings["n_repeats"],settings["L1"]))
    start_time = time.time()
    performance[r_nr] = model.crossvalidated_regression( repeats=settings["n_repeats"], L1=settings["L1"], nonnegative=settings["nonnegative"], withinrange=settings["withinrange"], shuffle=settings["shuffle Y"], shuffle_trials=settings["shuffle trials"], shuffle_categories=settings["shuffle categories"], swap_categories=settings["swap categories"], return_var_per_group=CAencodingmodel.PREDICTOR_GROUPS, output_var=settings["output variable"], rec=rec, crec=crec, progress_indicator=args.verbose)
    print("     -> running time: {:0.2f} s".format(time.time()-start_time))
    print("    Maximum R2: {:0.3f}".format( np.nanmax(np.nanmean(performance[r_nr]["Full"],axis=0)) ))
